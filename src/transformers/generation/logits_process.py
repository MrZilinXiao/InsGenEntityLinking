    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for batch_id, beam_sent in enumerate(input_ids.view(-1, self._num_beams, input_ids.shape[-1])):
            for beam_id, sent in enumerate(beam_sent):
                allowed_token_ids = self._prefix_allowed_tokens_fn(batch_id, sent)
                if self._prefix_allowed_tokens_fn.debug:
                    allowed_scores = {token_id: scores[batch_id * self._num_beams + beam_id, token_id] for token_id in allowed_token_ids}
                    print(f"in the beam No. {beam_id}, allowed tokens and their scores: {allowed_scores}\n")
